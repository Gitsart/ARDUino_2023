...
void push_button()
{
  if  ((digitalRead(Emergency) == HIGH) && (digitalRead(front_pb) == LOW) || (digitalRead(back_pb) == LOW) && battery_low == false && Start_process == false && pause == false && Back_Start_process == false )
  {
    front_cam = true;
    back_cam = false;
    count = 0;
  ...
  else if ((digitalRead(Emergency) == HIGH) && (digitalRead(front_pb) == LOW) || (digitalRead(back_pb) == LOW)  && battery_low == false && Back_Start_process == false && pause == false && Start_process == false)
  {
    front_cam = false;
    back_cam = true;
    count = 0;
...
//  
  ...
  if (currentMillis - previousMillis >= period)
  {
    ...
    {
      front_cam_loop();
      Serial.print("sig"); Serial.println(signature);
    }
    else if (back_cam == true)
    {
      back_cam_loop();
      Serial.print("sigb"); Serial.println(signature_b);
    }
  }
  front_obstruction   = digitalRead(front_scanner);
  back_obstruction  = digitalRead(back_scanner);
